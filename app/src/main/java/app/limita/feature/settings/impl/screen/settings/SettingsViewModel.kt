/*
 * Copyright (C) 2024, Usmonjon Abdurakhmanov and the Limită project contributors
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published
 * by the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <https://www.gnu.org/licenses/>.
 */

package app.limita.feature.settings.impl.screen.settings

import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import app.limita.feature.settings.impl.screen.settings.state.SettingsAction
import app.limita.feature.settings.impl.screen.settings.state.SettingsAction.NavigateBack
import app.limita.feature.settings.impl.screen.settings.state.SettingsAction.NavigateToAbout
import app.limita.feature.settings.impl.screen.settings.state.SettingsSideEffect
import kotlinx.coroutines.channels.Channel
import kotlinx.coroutines.flow.Flow
import kotlinx.coroutines.flow.receiveAsFlow
import kotlinx.coroutines.launch

internal class SettingsViewModel : ViewModel() {

    private val sideEffectChannel = Channel<SettingsSideEffect>()
    val sideEffects: Flow<SettingsSideEffect> = sideEffectChannel.receiveAsFlow()

    fun onAction(action: SettingsAction) = when (action) {
        NavigateBack -> navigateBack()
        NavigateToAbout -> navigateToAbout()
    }

    private fun navigateBack() = viewModelScope.launch {
        sideEffectChannel.send(SettingsSideEffect.NavigateBack)
    }

    private fun navigateToAbout() = viewModelScope.launch {
        sideEffectChannel.send(SettingsSideEffect.NavigateToAbout)
    }
}
